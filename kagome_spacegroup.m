function [group, bonds, pos_cart] = kagome_spacegroup(a, b)
%KAGOME_SPACEGROUP  Space group of a C_6-symmetric kagome torus, as a
%                   site-permutation group, with nearest-neighbour bonds.
%
%   [GROUP, BONDS, POS_CART] = KAGOME_SPACEGROUP(A, B) builds the kagome
%   lattice on a hexagonal (C_6-symmetric) supercell of the triangular
%   Bravais lattice spanned by
%       T1 = A*a1 + B*a2 ,   T2 = R6 * T1
%   (R6 = 60 deg rotation). The supercell holds |det| = A^2+A*B+B^2 unit
%   cells, each with 3 kagome sites -> N = 3*(A^2+A*B+B^2) sites. The full
%   space group is the translations (order = #cells) semidirect the point
%   group C_6v (order 12, about a hexagon centre at the origin), total order
%   12 * #cells. Examples: (A,B)=(2,0) -> N=12, |G|=48; (2,2) -> N=36, |G|=144.
%
%   Returned as the same struct contract as SQUARE_LATTICE_SPACEGROUP /
%   ICOSAHEDRON_IH_FULL (perms, mul, inv, identity, class_idx/size), so
%   IRREPS_FROM_GROUP + the FTLM pipeline consume it unchanged. BONDS is the
%   [2N x 2] NN bond list (each site has 4 neighbours -> 2N bonds). POS_CART
%   is [N x 2] Cartesian site positions (for plotting / sanity).
%
%   Convention: work in (a1,a2)-COEFFICIENT coordinates scaled by 2 (so all
%   site coords are integers w = 2*p). Point-group elements are integer
%   matrices in this basis: R6 = [0 -1; 1 1], sigma = [1 1; 0 -1].
%
%   See also SQUARE_LATTICE_SPACEGROUP, IRREPS_FROM_GROUP, MIN_IMAGE_IH.
...
% ================================================================
    ...
    %% 3 kagome basis sites in 2*(a1,a2)-coeff (integer): a1/2,a2/2,(a1+a2)/2.
    basis_w = [1 0; 0 1; 1 1];         % rows; = 2 * basis coeff
    ...
    assert(size(site_w,1) == N, 'kagome_spacegroup: found %d sites, expected %d', size(site_w,1), N);
    ...
    %  ops (R6, sigma) that PRESERVE this supercell lattice (Tm\(M*Tm) integer).
    %  R6 always passes (T2 = R6*T1 by construction) -> at least C_6; the mirror
    %  sigma passes only for NON-CHIRAL cells (b==0 or a==b) -> C_6v. Without this
    %  filter an invalid sigma on a chiral cell blows the BFS closure up to a huge
    %  non-crystallographic group (see triangular_spacegroup for the same fix).
    I2 = eye(2);
    ...
    %% BFS closure -> all |G| site permutations.
    ...
    order = size(perms,1);
    %% SOFT order check: translations are always faithful (free), so
    %  order = ncells * (faithful point-group order). Non-chiral cells -> C_6v
    %  (12*ncells); chiral cells drop the mirror -> C_6 (6*ncells). Either is a
    %  valid symmetry group (sum rule / ED stay exact), so warn, do not error.
    assert(mod(order, ncells) == 0, ...
        'kagome_spacegroup: closure order %d not a multiple of ncells %d', order, ncells);
    ...
    if pg_order < 12
        warning('kagome_spacegroup:partialPG', ...
            ['site-permutation action faithful only for a %s point group (|G|=%d; ' ...
             'full C_6v would be 12*%d=%d) -- still a valid symmetry group.'], ...
    ...
    %% Nearest-neighbour bonds (Cartesian distance = 0.5 under PBC).
    bonds = build_bonds(pos_cart, Tcart);
...
function bonds = build_bonds(pos, Tcart)
    N = size(pos,1);  d0 = 0.5;  tol = 1e-6;  bonds = zeros(0,2);
    shifts = [];
    ...
    assert(size(bonds,1) == 2*N, 'build_bonds: %d bonds != 2N=%d', size(bonds,1), 2*N);
end
